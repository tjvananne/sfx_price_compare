...
if(!GBL_tbl_name_ASIN %in% db_list_tables(p_conn=conn, db_secret$database)) {
    dbExecute(conn=conn, statement=
    paste0(
        "CREATE TABLE ", GBL_tbl_name_ASIN, " (
        ASIN_id INT NOT NULL IDENTITY(1,1) PRIMARY KEY,
        ASIN varchar(15) NOT NULL,
        CONSTRAINT ", GBL_env, "_UNQ_ASIN UNIQUE (ASIN));"))
}

 
# db_drop_table(conn, "ASIN")
if(!GBL_tbl_name_ASIN_Category %in% db_list_tables(p_conn=conn, db_secret$database)) {
    dbExecute(conn=conn, statement=
    paste0(
        "CREATE TABLE ", GBL_tbl_name_ASIN_Category, " (
        ASIN_id INT NOT NULL CONSTRAINT ", GBL_env, "_FK_ASIN_id_Category REFERENCES ", GBL_env, "_ASIN(ASIN_id),
        ASIN varchar(15) NOT NULL,
        Category1 varchar(150) NOT NULL,
        Category2 varchar(150) NOT NULL,
        Category3 varchar(150),
        CONSTRAINT ", GBL_env, "_UNQ_ASIN_Category UNIQUE (ASIN, Category1, Category2));"))
}
...
if(!GBL_tbl_name_Amz_Product %in% db_list_tables(p_conn=conn, db_secret$database)) {
    dbExecute(conn=conn, statement=
    
    paste0(
        "CREATE TABLE ", GBL_tbl_name_Amz_Product, " (
        ASIN_id INT NOT NULL CONSTRAINT ", GBL_env, "_FK_ASIN_id_Product REFERENCES ", GBL_env, "_ASIN(ASIN_id),
...
if(!GBL_tbl_name_Amz_ListPrice %in% db_list_tables(p_conn=conn, db_secret$database)) {
    ...
    dbExecute(conn=conn, statement=
        paste0(
            "CREATE TABLE ", GBL_tbl_name_Amz_ListPrice, " (
